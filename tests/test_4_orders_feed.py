...
class TestOrdersFeed:
    ...
    @allure.description('При клике на заказ в разделе "Лента заказов" появляется всплывающее окно с информацией по этому заказу')
    ...
    @allure.description('Номер заказа пользователя, который отображается в разделе "История заказов" в личном кабинете соответствует номеру заказа из раздела "Лента заказаов"')
    ...
    @allure.description('Показатель счетчика "Выполнено за всё время" на странице Лента заказов до оформления заказа пользователем будет меньше, показатель чем после оформления заказа')
